...
async function inspectAgreements() {
    try {
        ...
        if (snapshot.exists()) {
            ...
            console.log('--- DETAILED RENTAL AGREEMENTS ---');
            Object.entries(data).forEach(([id, ag]) => {
                console.log(JSON.stringify({
                    id,
                    assetName: ag.assetDetails?.assetName,
                    category: ag.assetCategory,
                    status: ag.inspectionWorkflow?.status,
                    tech: ag.inspectionWorkflow?.technicianId,
                    serial: ag.assetDetails?.serialNumber,
                    vin: ag.assetDetails?.vin
                }, null, 2));
            });
        }
    } catch (err) {
